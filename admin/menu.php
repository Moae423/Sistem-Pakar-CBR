<nav class="nav-bg navbar  navbar-expand-lg navbar-dark p-md- ">
	<div class="container">
		<a class="puskesmas fs-2" href="#">
			<img src="../admin\image\logo-puskesmas.png" width="30" height="30" class="d-inline-block align-text-top" />
			<a href="index.php?top=home.php	" class="puskesmas fs-1">
				Puskesmas Padang Luar
			</a>
		</a>
		<button type="button" class="navbar-toggler" data-bs-target="#navbarNav" data-bs-toggle="collapse"
			aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle Navbar">
			<span class="navbar-toggler-icon"></span>
		</button>
		<div class="collapse navbar-collapse" id="navbarNav">
			<div class="mx-auto"></div>
			<ul class="navbar-nav fs-5 gap-5">
				<li class="nav-item">
					<a href="haladmin.php?top=home.php" class="nav-ateh">Home</a>
				</li>
				<li class="nav-item">
					<a href="haladmin.php?top=penyakit_solusi.php" class="nav-ateh">Penyakit & Solusi </a>
				</li>
				<li class="nav-item">
					<a href="haladmin.php?top=gejala.php" class="nav-ateh">Gejala</a>
				</li>
				<li class="nav-item">
					<a href="haladmin.php?top=relasi.php" class="nav-ateh">Relasi</a>
				</li>
				<li class="nav-item">
					<a href="haladmin.php?top=lapgejala.php" class="nav-ateh">Laporan Gejala</a>
				</li>
				<li class="nav-item">
					<a href="haladmin.php?top=lapuser.php" class="nav-ateh">Laporan User</a>
				</li>
				<li class="nav-item">
					<a href="logout.php" class="nav-ateh">Logout</a>
				</li>

			</ul>
		</div>
	</div>
</nav>